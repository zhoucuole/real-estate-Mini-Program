...
Page({
  ...
  // 房源类型选择
  typeDecChange: function (e) {
    var id = e.currentTarget.dataset.id
    var check = e.currentTarget.dataset.check
    var list = this.data.typeDecList
    list.map(function (e) {
      if (e.id == id) {
        e.check = true
      } else {
  ...
  formSubmit: function (e) {
    ...
    var typeDecList = ''
    this.data.typeDecList.map(function (a) { //获取装修的id
      if (a.check) {
        console.log(a)
        typeDecList = a.id
      }
    ...
    if (addressArr.ad_info.district != '南沙区') {
      ...
      wx.request({
        ...
        data: {
          ...
          area: obj.area, //面积
          price: obj.price, //价格
          unit_price: obj.fprice,
          property: obj.property, //物业
          address: addressArr.address, //地址
          decoration: typeList, // 装修
          is_commission: obj.check, //佣金
          commission: obj.dprice,
          variety: typeDecList, //类型 
          contact: obj.name, //纤细人
          phone: obj.phone, //联系电话
          floor: parseInt(obj.floorData) + 1, //楼层
